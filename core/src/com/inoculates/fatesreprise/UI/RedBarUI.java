...
public class RedBarUI extends UI {
    GameScreen screen;
    ...
    BitmapFont firstSlot, secondSlot, thirdSlot, money;
    ShaderProgram fontShader;
    RedBar bar;

    public RedBarUI(GameScreen screen, TextureAtlas atlas) {
        super(atlas.findRegion("WhiteUI"));
        this.screen = screen;
        ...
        bar = new RedBar(screen, atlas, this);
        screen.UIS.add(bar);
        createText();
    }

    // Draws allt he different texts.
    public void draw(Batch batch) {
        super.draw(batch);
        batch.setShader(fontShader);
        // Text telling user how much money he has.
        money.setScale(0.3f);
        money.draw(batch, Integer.toString(screen.storage.coins), getX() + 95, getY() + money.getCapHeight() - 1.5f);
        ...
        if (screen.storage.item1 != null) {
            screen.storage.item1.setPosition(getX() + 11.5f, getY() + 3);
            screen.storage.item1.draw(batch);
        }
        if (screen.storage.item2 != null) {
            screen.storage.item2.setPosition(getX() + 36.5f, getY() + 3);
            screen.storage.item2.draw(batch);
        }
        if (screen.storage.item3 != null) {
            screen.storage.item3.setPosition(getX() + 61.5f, getY() + 3);
    ...
    // This converts every unconvertable key into text proper. (For example mouse button 1, enter, etc).
